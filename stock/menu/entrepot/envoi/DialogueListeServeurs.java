 ...
 */
public class DialogueListeServeurs extends javax.swing.JDialog {
    private FenetreEntrepotEnvoi fenMere;
    private List<Serveur> listServeurLocale = new ArrayList<Serveur>();
    
    /**
     * Creates new form NewJDialog
     */
    public DialogueListeServeurs(java.awt.Frame parent, boolean modal, String title, List<Serveur> listServeur) {
        super(parent, modal);
        initComponents();
        ...
        this.listServeurLocale = listServeur;
        
        //Change le titre de la fenetre
        this.labelNomFenetre.setText(title);
        
        // Affichage de chaque serveur de la liste
        int i;
        for (i = 0; i < listServeur.size(); i++)
        {
            afficheServeur(listServeur.get(i));
        }
    }
    
    public DialogueListeServeurs(java.awt.Frame parent, boolean modal, int longueur, int largeur, String title, List<Serveur> listServeur) {
        this(parent, modal, title, listServeur);
    ...
    private void initComponents() {
        ...
        tabServeurs.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mousePressed(java.awt.event.MouseEvent evt) {
                tabServeursMousePressed(evt);
            }
        });
    ...
    private void btnSupprMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btnSupprMouseClicked
        if(this.btnSuppr.isEnabled())
        {
            ...
            //Supprimer le Serveur sélectionné si confirmation
            retirerServeurSelectionne();

        }
    }//GEN-LAST:event_btnSupprMouseClicked

    private void tabServeursMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabServeursMousePressed
        this.btnSuppr.setEnabled(true);
    }//GEN-LAST:event_tabServeursMousePressed
    

    /**
     * @brief Affiche dans la JTable de la fenetre le serveur passe en parametre (place en fin de liste)
     * 
     * @param Serveur à ajouter dans la liste d'affichage
     */
    ...
    /**
     * @brief Retire le serveur actuellement selectionné du conteneur en cours
     */   
    private void retirerServeurSelectionne()
    {
        ...
        int indexSelectedRow = this.tabServeurs.convertRowIndexToModel(this.tabServeurs.getSelectedRow());
        dtm.removeRow(indexSelectedRow);
        dtm.fireTableRowsDeleted(indexSelectedRow - 1, indexSelectedRow - 1);
        
        // Supprime le serveur selectionne de la liste du conteneur selectionne
        Serveur selectedServeur = listServeurLocale.get(indexSelectedRow);
        this.fenMere.retirerServeurSelection(selectedServeur);
        
        // Retirer ce Serveur du conteneur dans la BDD
//////////////////          A FAIRE         ////////////////////////////////////
